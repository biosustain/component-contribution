...
MOL_FILES_DIR = "../examples/molfiles/"
...
class ComponentContribution(object):
    ...
    def _decompose_reaction(self, reaction):
        ...
        for compound_id, coeff in reaction.iteritems():
            ...
            if reaction.format == 'mol':
                x_prime.append(coeff)

                test_file = open(MOL_FILES_DIR+compound_id, 'r')
                mol_file = test_file.read()


                inchiS = consistent_to_inchi(mol_file, 'mol')
                comp = Compound.from_inchi_with_keggID('mol', None, inchiS)
