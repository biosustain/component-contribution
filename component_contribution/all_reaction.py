...
class AllReaction(object):

    def __init__(self, sparse, format='bigg', arrow='<=>', rid=None):
    ...
    @staticmethod
    def parse_formula(formula, format, arrow='<=>', rid=None):
        """ 
        ...
        sparse_reaction = {}
        for met_id, count in AllReaction.parse_reaction_formula_side(left).iteritems():
            sparse_reaction[met_id] = sparse_reaction.get(met_id, 0) - count

        for met_id, count in AllReaction.parse_reaction_formula_side(right).iteritems():
            sparse_reaction[met_id] = sparse_reaction.get(met_id, 0) + count

        return AllReaction(sparse_reaction, format, arrow, rid)
    ...
    def write_formula(self):
        ...
        for cid, coeff in sorted(self.sparse.iteritems()):
            if coeff < 0:
                left.append(AllReaction.write_compound_and_coeff(cid, -coeff))
            elif coeff > 0:
                right.append(AllReaction.write_compound_and_coeff(cid, coeff))
...
if __name__ == '__main__':
    reaction = BiggReaction.parse_formula('C00149 <=> C00036', )
