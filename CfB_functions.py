...
from component_contribution.thermodynamics.constants import STANDARD_T, STANDARD_RT
...
def decompose_without_cache(cc,comp):
    ...
    try:
        group_vec = cc.decomposer.smiles_to_group_vector(comp.smiles_pH7)
        g = numpy.matrix(group_vec.to_array())
...
def formation_energy(listMolecules, cc, pH=7, I=0.1, T=298.15, format_molecule='kegg'):
    ...
    for molecule_input in listMolecules:
        ...
        # print d['dG0_f']
        a = comp.get_species(major_ms_dg0_f, STANDARD_T)
        dG0_f = a.next()['dG0_f']
    ...
    for met in input_metabolites:
        ...
        if all_references_json == 'its not bigg':
            ...
            if os.path.isfile('../examples/molfiles/' + met):
                ...
                cid, Name = check_if_already_exists_inchi(inchiS)
                if cid == None:
                    comp = Compound.from_inchi('MOL', met, inchiS)
                    comp.molfile = mol
                    # save the references
...
def calc_concentration(calculate_conc, input_metabolites, comp_data=None, ccache=None):
    '''
    input is the metabolites in the stoichiometric matrix of the thermo prediction_model. These are already converted to keggIds
    :return:
    '''
    import math
    from component_contribution.core.molecule import Molecule
    from component_contribution.compound_cache import CompoundCache
    from CfB_functions import process_input_mets
    ...
    for compound_id,comp in zip(comp_data_mapper.keys(), comp_data_mapper.values()):
        ...
        if calculate_conc:
            ...
            # calc NPSA ##
            NPSA1 = non_polar_surface_area(comp.inchi, pH=7)
    ...
    if 'CHEBI' in info:
        ...
        try:
            if res.ChemicalStructures[0].type == 'mol':
                ...
                try:
                    compound = ccahce.get_compound(cid)
                except:
                    compound = Compound.from_inchi(inchi, 'CHEBI', cid)
...
def output_reaction_energies(in_filename, pH=7, I=0.1, out_filename='test'):
    ...
    model = MetabolicModel.from_formulas(reaction_strings, 'bigg', raise_exception=False)
    ...
    with open(out_filename, "w") as text_file:
        text_file.write('reaction formula' + '\t' + 'prediction_model.dG0' +
                        '\t' + 'dG0_prime' + '\t' + 'dGm_prime' + '\t' + 'dG0_std' + '\n')
        ...
        for i, rxn_str in enumerate(reaction_strings):
            ...
            text_file.write(rxn_str.strip() + '\t' + str(model.dG0[i, 0]) + '\t' + str(dG0_prime[i, 0]) + '\t' +
                            str(dGm_prime[i, 0]) + '\t' + str(dG0_std[i, 0]*Z) + '\n')