...
# Test group_vector.py
try:
    from component_contribution.core import group_vector

    groups_data = group_vector.init_groups_data()
    decomposer = group_vector.InChIDecomposer(groups_data)
    groupvec1 = decomposer.inchi_to_group_vector(inchi_ATP)
    groupvec2 = decomposer.smiles_to_group_vector(smiles_ATP_pH7)
    print(groupvec1)
    print(groupvec2)
    ...
    sys.stderr.write('Cannot import the python script inchi2gv. Make sure the file '
                     'group_vector.py is located in the folder '
